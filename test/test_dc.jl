...
@testset "Distributional Clustering Tests" begin
    output = ""
    @testset "Distributional Clustering Basic Functionality" begin
        ...
        for testCase in testCases
            @testset "Test Case: $(testCase.name)" begin
                ...
                # Create and fit Distributional Clustering model
                model = DC(k=num_classes)
    ...
    @testset "Argument Validation Tests" begin
        @test_throws ArgumentError DC(k=0) # k must be at least 1
        @test_throws ArgumentError DC(k=-1) # k must be a positive integer
        @test_throws ArgumentError DC(k=2, max_try=-10) # max_try must be non-negative
        @test_throws ArgumentError DC(k=2, tol=-0.1) # tol must be non-negative
        @test_throws TypeError DC(k=2, tol="not_a_number") # tol must be a number
        @test_throws TypeError DC(k=2, max_try="not_a_number") # max_try must be an integer
        @test_throws TypeError DC(k="not_a_number") # k must be an integer
    end

    @testset "Distributional Clustering no print output" begin
        @test isempty(output)
    end
end